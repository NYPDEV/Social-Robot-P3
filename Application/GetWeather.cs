...
namespace SocialRobot.Application
{
    public class GetWeather
    {
        ...
        public void GetTheWeather(string Country, string Day)
        {
            ...
            //}
            #region weather
            //http://api.openweathermap.org/data/2.5/forecast/daily?q=London,us&mode=xml&appid=b3c579db28aa6eec025acbaaf78c2976
            //http://api.openweathermap.org/data/2.5/forecast?q=London,us&mode=xml&appid=b3c579db28aa6eec025acbaaf78c2976
            //http://api.openweathermap.org/data/2.5/weather?q=London,us&mode=xml&appid=b3c579db28aa6eec025acbaaf78c2976

            string query2 = String.Format("http://api.openweathermap.org/data/2.5/forecast/?q=" + Country + "&units=metric&mode=xml&appid=" + ApiKey);
            XmlDocument wData = new XmlDocument();
            try
            {
                wData.Load(query2);
        ...
        #endregion GetTheWeather

        #region GetTheWeatherForeCast
        public void GetTheWeatherForeCast(string Country, DateTime Date_Time)
        {
            List<string> from_weather = new List<string>() { };
            List<string> to_weather = new List<string>() { };
            List<string> temperature = new List<string>() { };
            List<string> max_temperature = new List<string>() { };
            List<string> min_temperature = new List<string>() { };
            List<string> rain = new List<string>() { };
            List<string> symbolID = new List<string>() { };
            string rain1;
            ...
            string query2 = String.Format("http://api.openweathermap.org/data/2.5/forecast/daily?q=" + Country + "&units=metric&mode=xml&appid=" + ApiKey);
            XmlDocument wData = new XmlDocument();
            try
            {
                wData.Load(query2);
            ...
            try
            {
                ...
                foreach (XmlNode node in forecast)
                {
                    var temp = node.SelectSingleNode("temperature").Attributes["day"].Value;
                    var min_temp = node.SelectSingleNode("temperature").Attributes["min"].Value;
                    var max_temp= node.SelectSingleNode("temperature").Attributes["max"].Value; 
                    if (Convert.ToDouble(temp) >= 100)
                    {
                        double tempInt = Convert.ToDouble(temp) / 10;
                        temp = tempInt.ToString();
                    ...
                    temperature.Add(temp);
                    max_temperature.Add(max_temp);
                    min_temperature.Add(min_temp);
                    //check if there is rain
                    try
                    {
                        var pep = node.SelectSingleNode("precipitation").Attributes["value"].Value;
                        var type = node.SelectSingleNode("precipitation").Attributes["type"].Value;
                        rain.Add(pep);
                    }
                ...
                try
                {
                    if (Date_Time.Date == DateTime.Today.Date)
                    {
                        ...
                        {
                            weathericon(symbolID[1]);
                            TTS.Speaking("the weather condition in " + Country + " for " + "today" + " is " + (int)temp + "  degrees celsius " + WeatherCondition + "." + "\r\n" + "The maximum temperature is " + Convert.ToDouble(max_temperature[1]) + " and the minimum temperature is  " + Convert.ToDouble(min_temperature[1])); Country = "";
                        }
                        else
                        {
                            weathericon(symbolID[1]);
                    ...
                    else if (Date_Time.Date == DateTime.Today.Date.AddDays(1))
                    {
                        double temp = 0;
                        temp = Convert.ToDouble(temperature[3]);
                        if (rain[3] == "No rain")
                        {
                            weathericon(symbolID[3]);
                            TTS.Speaking("The weather condition in " + Country + " for " + "tomorrow" + " is  " + (int)temp + " degrees celsius " + WeatherCondition
                               + "." + "\r\n" + "The maximum temperature is " + Convert.ToDouble(max_temperature[1]) + "  degrees celsius " + " and the minimum temperature is " + Convert.ToDouble(min_temperature[1]) + "  degrees celsius "); Country = "";
                        }
                        else
                        {
                            weathericon(symbolID[3]);
                    ...
                    //else 
                        if (Date_Time == DateTime.Now)
                        {
                            ...
                            {
                                weathericon(symbolID[1]);
                                TTS.Speaking("the weather condition in " + Country + " for " + "Now" + " is " + (int)temp + " degrees celsius " + WeatherCondition + ".  The maximum temperature is" + Convert.ToDouble(max_temperature[1])); Country = "";
                            }
                            else
                            {
                                weathericon(symbolID[1]);
                                TTS.Speaking("the weather condition in " + Country + " for " + "Now" + " is " + (int)temp + " degrees celsius " + WeatherCondition + " now."); Country = "";
                            }
                            OnWeatherTextUpdate?.Invoke(temp);
                        }
                }
                ...
                {
                    TTS.Speaking("I am having trouble retriving the weather forecast for " + Country + ". please try again.");
                    Country = "";
                }
            }
            catch
            {
                ...
                else
                {
                    if (Country != "")
                    {
                        TTS.Speaking("So sorry, I can not get the weather condition in " + Country + " now."); Country = "";
                    }
                    else
                    {
                        TTS.Speaking("sorry I did not hear clearly, please try again!");
            ...
            #endregion GetTheWeatherForeCast
        }
        ...
        public void weathericon(string icon)
        {
            ...
            switch (icon)
            {
                #region thunderstorm
                case "200":
                    WeatherCondition = " and there will be a thunderstorm with light rain ";
                ...
                case "201":
                    WeatherCondition = " and here will be a thunderstorm with rain ";
                    WeatherBit.BeginInit();
                ...
                case "202":
                    WeatherCondition = " and there will be a thunderstorm with heavy rain ";
                    WeatherBit.BeginInit();
                ...
                case "210":
                    WeatherCondition = " and there will be a light thunderstorm ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "211":
                    WeatherCondition = " and there will be a thunderstorm ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "212":
                    WeatherCondition = " and there will be a heavy thunderstorm ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "221":
                    WeatherCondition = " and there will be a ragged thunderstorm ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "230":
                    WeatherCondition = " and there will be a thunderstorm with light drizzle ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "231":
                    WeatherCondition = " and there will be a thunderstorm with drizzle ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "232":
                    WeatherCondition = " and there will be a thunderstorm with heavy drizzle ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\PM Thunderstorms.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                #region drizzle
                case "300":
                    WeatherCondition = "and there will be a light intensity drizzle";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "301":
                    WeatherCondition = "and there will be a drizzle";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "302":
                    WeatherCondition = "and there will be a heavy intensity drizzle";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "310":
                    WeatherCondition = "and there will be a light intensity drizzle rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "311":
                    WeatherCondition = "and there will be a drizzle rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "312":
                    WeatherCondition = "and there will be a heavy intensity drizzle rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "313":
                    WeatherCondition = "and there will be a shower rain and drizzle";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "314":
                    WeatherCondition = "and there will be a heavy shower rain and drizzle";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "321":
                    WeatherCondition = "and there will be a shower drizzle";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                #region rain
                case "500":
                    WeatherCondition = "and there will be a light rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "501":
                    WeatherCondition = "and there will be a moderate rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "502":
                    WeatherCondition = "and there will be a heavy intensity rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\5.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "503":
                    WeatherCondition = "and there will be a very heavy rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\5.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "504":
                    WeatherCondition = "and there will be a extreme rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\5.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "511":
                    WeatherCondition = "and there will be a freezing rain";
                    WeatherBit.BeginInit();
                ...
                case "520":
                    WeatherCondition = "and there will be a light intensity shower rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "521":
                    WeatherCondition = "and there will be a shower rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Showers.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "522":
                    WeatherCondition = "and there will be a heavy intensity shower rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\5.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "531":
                    WeatherCondition = "and there will be a ragged shower rain";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\5.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                #region clouds
                case "800":
                    WeatherCondition = " with clear sky ";
                    WeatherBit.BeginInit();
                ...
                case "801":
                    WeatherCondition = " with few clouds ";
                    WeatherBit.BeginInit();
                ...
                case "802":
                    WeatherCondition = " with scattered clouds ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Partly Cloudy.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "803":
                    WeatherCondition = " with broken clouds ";
                    WeatherBit.BeginInit();
                ...
                case "804":
                    WeatherCondition = " with overcast clouds ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\Partly Cloudy.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                #region extreme
                case "900":
                    WeatherCondition = " and there will be a tornado ";
                    WeatherBit.BeginInit();
                    WeatherBit.UriSource = new Uri(AppDomain.CurrentDomain.BaseDirectory + @"weather-icons-set\16.png");
                    WeatherBit.EndInit();
                    OnWeatherIconUpdate?.Invoke(WeatherBit);
                ...
                case "901":
                    WeatherCondition = " and there will be a tropical storm ";
                    break;

                case "902":
                    WeatherCondition = " and there will be a hurricane ";
                    break;

                case "903":
                    WeatherCondition = " and the weather will be extremely cold ";
                    break;

                case "904":
                    WeatherCondition = " and the weather will be extremely hot ";
                    break;

                case "905":
                    WeatherCondition = " and the weather will be extremely windy ";
                    break;

                case "906":
                    WeatherCondition = " and there will be hail ";
                    break;
                #endregion extreme
                #region additional
                case "951":
